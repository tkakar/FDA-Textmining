...
exp2 = "(this|next|last)"
iso = "\d+[/-]\d+[/-]\d+ \d+:\d+:\d+\.\d+"
unk = "((?:unknown|unspecified) dates?)"
# updated this to only have it select years that are preceded by letter than space (i.e. In 2015)
# TODO make it include characters that aren't letters (i.e. In, 2015)
year = "((?<=[a-zA-Z]\s)\d{4}|^\d{4})"
...
month_abbrev_named = "(jan(?:uary)?|feb(?:uary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sept(?:ember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)"
date_num_named = "(([0-9]{1,2}))"
month_num_named = "(([0-9]{1,2}))"
year_named = "((?<=\s)\d{4}|^\d{4})"
year_named_month = "((?<=(?:/|-))\d{4}|^\d{4})"

space_or_comma = "(?:(?:\s|,)?)+"
space_or_comma_dash = "(?:(?:\s|,|/|-)?)+"
# IMPORTANT: Always creates a group!, and in the future, takes the first group
# mar(ch) 23(,) 2015
...
# mm/dd/yyyy
mmddyyyy = "(" + month_num_named + "(?:/|-)" + date_num_named + "(?:/|-)" + year_named_month + ")"
# dd month yyyy or dd-month-yyyy
ddmonthyyyy = "(" + date_num_named + space_or_comma_dash + month_abbrev_named + space_or_comma_dash + year_named_month + ")"
...
reg10 = re.compile(month_day_year_alpha, re.IGNORECASE)
reg11 = re.compile(month_year_alpha, re.IGNORECASE)
reg12 = re.compile(mmddyyyy)
reg13 = re.compile(ddmonthyyyy, re.IGNORECASE)


# print('month_year: '  + month_year_alpha)
# print('mmddyyyy: ' + mmddyyyy)
# print ('ddmonthyyyy: ' + ddmonthyyyy)

def tag(text):
    # Initialization
    timex_found = []
    # year needs its own special case, so it doesn't overflow with date and year
    timex_found2 = []
    ...
    #    found = [m.groupdict() for m in reg10.finditer(text)]
    #    print(found)
...
def demo():
    # sample inputs
    sample = ["On mar 23, 2015, I ate food.",
